...
/**
 * Shared rendering for the public tracking flow. Used by both
 * `/track/[token]` (deep-link) and `/track` (form-driven). Centralising
 * here keeps the visual presentation identical regardless of how the
 * customer arrived at the tracking data — and gives us one place to
 * update the copy/instructions when the carrier flow changes.
 */
import {
  ArrowUpRight,
  CheckCircle2,
    ...
    <Card className="border-[color:var(--accent)]/30 bg-[color:var(--accent-soft)]/30">
      ...
      <CardContent className="flex flex-col gap-4">
        ...
        {(data.orderStatus ?? "").toUpperCase() === "DELIVERED" ? (
          // Seller has marked the order DELIVERED. The carrier's tracking
          // page sometimes lags behind reality, so we keep the AWB + open
          // button visible AND surface the right escalation path: contact
          // the carrier's local delivery partner, not the seller.
 ...
 * processed yet so callers can drop it in unconditionally. Deliberately
 * omits the seller-side proof screenshot (private) and the audit trail —
 * the customer just needs amount, when it was refunded, and the Razorpay
 * refund id (so they can match it against their bank statement).
 */
export function RefundCard({
  ...
  data: PublicTracking;
  /**
   * Endpoint that streams the seller-uploaded refund proof image. Caller
   * decides which lookup key to use (token vs orderNo). Image renders
   * only when both `refund.hasProof` is true and a URL is provided.
   */
  proofUrl?: string;
}) {
    ...
    <div className="min-h-screen bg-background">
      <main className="mx-auto flex w-full max-w-2xl flex-col gap-4 p-3 sm:p-6">
        <header className="flex items-center justify-between py-2">
          ...
          <span className="text-[11px] text-muted-foreground">Order tracking</span>
        </header>
        {children}
      </main>
    </div>
